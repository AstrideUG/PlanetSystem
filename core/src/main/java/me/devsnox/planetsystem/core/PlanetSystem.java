...
public class PlanetSystem extends JavaPlugin {
    ...
    public void saveAll() {
        ...
        if (planets.size() != 0) planets.forEach(planet -> holder.getPlanetData().save(planet.getOwnerUniqueID()));
        if (players.size() != 0) players.forEach(player -> holder.getPlayerData().save(player.getUUID()));
    }
